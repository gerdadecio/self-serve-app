...
export default ({ task }) => {
  ...
  return (
    <div className="container">
      ...
      <h5 className="section-header">Item Summary</h5>
      <div className="summary">
        <div>
          <small className="text-muted">
            <label>Marketplace:</label> {item.marketplace_name} <br/>
            <label>External ID:</label> {item.external_id} <br/>
            <label>Created at:</label> {item.created_at} <br/>
            <label>Currency:</label> {item.currency} <br/>
            <label>Amount:</label> {parseFloat(Math.round((item.amount / 100.00) * 100) / 100).toFixed(2)} <br/>
            <label>Payment method:</label> {item.payment_method} <br/>
            <label>Payment type:</label> {item.payment_type_id} <br/>
            <label>Status:</label> {item.state} <br/>
            <label>Buyer:</label> {item.buyer_email} <br/>
            <label>Seller:</label> {item.seller_email} <br/>
          </small>
        </div>
      </div>
      <h5 className="section-header">Transactions</h5>
